...
export default function AboutHero({ data }) {
  const { tagline, title, subtitle, image } = data || {};

  return (
    <section className="relative w-full min-h-[70vh] flex flex-col justify-between overflow-hidden bg-luxury-black pt-32 pb-14 px-6 sm:px-12 lg:px-16 xl:px-20 select-none">
      {/* Night View Background with Orange Atmospheric Ambient Glow */}
      <div className="absolute inset-0 z-0">
        <img
          src={image || 'https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?auto=format&fit=crop&w=2560&q=85'}
          alt="Our Story - Country Holidays"
      ...
      <div className="relative z-10 max-w-4xl space-y-4 pt-4">
        <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-black/60 border border-orange-500/40 backdrop-blur-xl text-xs uppercase tracking-luxury text-orange-400 shadow-[0_0_20px_rgba(255,107,0,0.2)]">
          ...
          <span className="font-semibold text-white/90">{tagline || 'The Slowhouse Origin'}</span>
        </div>
        ...
        <p className="text-base sm:text-xl text-white/80 font-light max-w-2xl leading-relaxed">
          {subtitle || 'True luxury is not excess, but stillness, proportion, and immersive natural beauty.'}
        </p>
      </div>
      ...
      <div className="relative z-10 flex items-center space-x-3 pt-6">
        <div className="px-4 py-2 bg-black/55 backdrop-blur-xl border border-orange-500/30 rounded-full text-xs font-medium text-white flex items-center space-x-2">
          ...
          <span>Founded on Modern Architectural Monoliths</span>
        </div>
      </div>
    </section>
  );
